...
sea.flights <- filter(flights, flights$dest == "SEA")

# On average, did flights to seatac gain or loose time?
mean(sea.flights$gained, na.rm = TRUE)
lost
### Bonus ###
...
test <- select(flights, origin) %>% filter(origin == "JFK")
Specific <- function (org, destination) {
  return(select(flights, origin, dest, interest) %>% filter(origin == org, dest == destination))
