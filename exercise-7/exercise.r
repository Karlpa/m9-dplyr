...
team.data <- mutate(team.data, TTSR = TOV / STL)

# Sort the teams from lowest turnover/steal ratio to highest
team.data <- arrange(team.data, TTSR)
...
BetterShooters <- function(team1, team2) {
  return(filter(team.data, Team %in% c(team1, team2)) %>% filter(FG. == max(FG.)) %>% select(Name, FG., PTS))
}
better.shooters <- BetterShooters("Golden State Warriors*", "Boston Celtics*")
